...
#define WM_NEED_RESIZE WM_USER+100
...
LRESULT CALLBACK WindowProcedure (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam);
    ...
    LRESULT CALLBACK WindowProcedure (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam){
        if(___window == 0 && message != WM_SIZE){
	...
	//	return DefWindowProc (hwnd, message, wParam, lParam);
		switch (message)                  /* handle the messages */
        {
            ...
            case WM_CHAR:
                ...
                break;
            case WM_SIZE:
				PostMessage(hwnd, WM_NEED_RESIZE, wParam, lParam);
//                ___window->resizeEvent();
    ...
    void Window::checkForMessages(){
        ...
        if(PeekMessage (&msg, hwnd, 0, 0, PM_REMOVE)){
            ...
            switch(msg.message){
				case WM_NEED_RESIZE:
					std::cerr << "checkForMessages: " << message2String(msg.message) << " " << msg.message << std::endl;
					OutputDebugString("need resize");
                    break;
				case WM_SIZE:
					std::cerr << "checkForMessages: " << message2String(msg.message) << " " << msg.message << std::endl;
					OutputDebugString("reisze");
                    break;
                default:
					//std::cerr << "checkForMessages: " << message2String(msg.message) << " " << msg.message << std::endl;
                    DispatchMessage(&msg);
