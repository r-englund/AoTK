...
void Window::start(){
    ...
    __run = true;
    redisplay = true; //make sure atleast context is drawn once
    __start = clock();
    while(__run){
        checkForMessages();
        idleEvent();
...
unsigned int Window::addKeyUpListener(std::function<void (KEY key)> l){
    keyUpListenerFunctions[__keyUpId] = l;
    return __keyUpId++;
}
unsigned int Window::addKeyDownListener(std::function<void (KEY key)> l){
    keyDownListenerFunctions[__keyDownId] = l;
    return __keyDownId++;
}

unsigned int Window::addKeyImpulseListener(std::function<void (unsigned char key)> l){
...
unsigned int Window::addResizeListener(std::function<void(unsigned int, unsigned int)> l){
...
unsigned int Window::addMousePressListener(std::function<void(MOUSE_BUTTON mb,unsigned int x,unsigned int y)> l){
    mousePressListenerFunctions[__mousePressId] = l;
    return __mousePressId++;
}

unsigned int Window::addMouseReleaseListener(std::function<void(MOUSE_BUTTON mb,unsigned int x,unsigned int y)> l){
...
unsigned int Window::addMouseMotionListener(std::function<void(int dx,int dy)> l){
...
unsigned int Window::addPassiveMouseMotionListener(std::function<void(int dx,int dy)> l){
...
unsigned int Window::addScrollListener(std::function<void(int p)> l){
...
unsigned int Window::addIdleListener(std::function<void() > l){
