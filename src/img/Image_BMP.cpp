...
void Image<float>::saveBMP(std::string filename){
    ...
    for(int y = 0; y<infoheader.height;y++){
            ...
            for(int x = 0; x<infoheader.width;x++){
                ...
                PIXEL24BIT pixel;
                if(channels == 1){
                    pixel.red   = 255*clamp(data[0][i],0,1);
                    pixel.green = 255*clamp(data[0][i],0,1);
                    pixel.blue  = 255*clamp(data[0][i],0,1);
                }else{
                    pixel.red   = 255*clamp(data[0][i],0,1);
                    pixel.green = 255*clamp(data[1][i],0,1);
                    pixel.blue  = 255*clamp(data[2][i],0,1);
                }
                i++;
                fwrite(&pixel,sizeof(PIXEL24BIT),1,f);
            }
