...
template<typename T2> Image<T> Image<T>::convolution(const Image<T2> &img,_FILTER_WRAP wrap)const{
    if(!(std::is_same<T,float>::value || std::is_same<T,double>::value)){
        int status;
        std::cerr << "Warning, running convolution on a image thats not float or double, wrapping error might happen, conssider casting image to float or double before convoltution. Using("  << abi::__cxa_demangle(typeid(data[0][0]).name(),0,0,&status) << ")" << std::endl;
    }
